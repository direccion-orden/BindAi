...
async function run() {
  ...
  const closetProducts = products.filter(p => p.productType === "Closet" || (p.tags && p.tags.includes("Closet")) || p.title.includes("Closet") || p.title.includes("CLOSET") || p.title.includes("closet"));
