  ...
  return (
    <div className="flex flex-col space-y-6 max-w-5xl mx-auto pb-10">
      ...
      <div className="flex justify-between items-start">
        ...
        {invoice.status === "paid" && (
          <div className="px-4 py-2 bg-emerald-50 text-emerald-700 font-bold rounded-lg flex items-center gap-2 border border-emerald-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Columns: Bill Summary & Concepts */}
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card border rounded-xl p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase">Proveedor (Emisor)</p>
                <p className="font-bold text-slate-900 mt-0.5">{invoice.emisorName}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase">RFC</p>
                <p className="font-bold text-slate-900 mt-0.5">{invoice.emisorRfc}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase">Fecha Emisión</p>
                <p className="font-bold text-slate-900 mt-0.5">{invoice.date}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500 font-semibold uppercase">Total Facturado</p>
                <p className="font-black text-rose-600 mt-0.5">${(invoice.total || 0).toLocaleString("es-MX", { minimumFractionDigits: 2 })}</p>
              </div>
            </div>
          </div>
        ...
        {/* Right Column: Configurator & Confirmation Form */}
        <div className="space-y-6 col-span-1">
          <div className="bg-card border rounded-xl shadow-sm p-6 space-y-4">
            <h3 className="font-bold text-md border-b pb-2 flex items-center gap-2 text-rose-950">
              <DollarSign className="w-5 h-5 text-rose-600" />
              Configurar Pago (Egreso)
            </h3>

            <div className="bg-rose-50 text-rose-800 p-3 rounded-lg border border-rose-100 flex justify-between items-center text-sm">
              <span className="font-medium">Saldo Pendiente</span>
              <span className="font-black">${saldoPendiente.toLocaleString("es-MX", { minimumFractionDigits: 2 })}</span>
            </div>

            {saldoPendiente <= 0.01 ? (
              <div className="p-4 bg-emerald-50 text-emerald-800 border border-emerald-100 rounded-lg text-sm text-center font-bold">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <select 
                      ...
                      className="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-xs shadow-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                      required
                    >
                      {methods.map(m => (
                ...
                <div className="space-y-1">
                  ...
                  {vatRate > 0 && vatAccounts.length === 0 && (
                    <p className="text-[10px] text-rose-600 mt-0.5">Advertencia: No tienes una cuenta de IVA Acreditable Pagado (118) configurada.</p>
                  )}
                </div>
                ...
                <div className="pt-4 border-t flex justify-end gap-3">
                  <Button 
                    type="submit" 
                    disabled={saving} 
                    className="w-full bg-rose-600 hover:bg-rose-700 text-white gap-2 h-10 font-bold"
                  >
