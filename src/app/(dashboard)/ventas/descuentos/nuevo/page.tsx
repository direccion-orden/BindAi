...
import { Loader2, ArrowLeft, Tag, Percent, DollarSign, Calendar, Save, CheckCircle2 } from "lucide-react";
...
export default function NuevoDescuentoPage() {
  ...
  const toggleCategorySelection = (catId: string) => {
    if (targetIds.includes(catId)) {
      setTargetIds(targetIds.filter(id => id !== catId));
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const discountData = {
        ...
        status: "active", // Logic could set to scheduled if startDate is in future
        usageCount: 0,
        createdAt: new Date().toISOString()
      };

      await addDoc(collection(db, "companies", companyId, "discounts"), discountData);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Crear Descuento</h1>
          <p className="text-muted-foreground text-sm">
            Configura las reglas para tu nueva promoción
          </p>
        </div>
      </div>
