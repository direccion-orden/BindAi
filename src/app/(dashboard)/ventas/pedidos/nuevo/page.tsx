  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(orderRef, {
        id: orderId,
        orderNumber,
        quoteId: null, // Direct order without quote
        quoteNumber: null,
        ...
        <div className="md:col-span-1 space-y-6">
          ...
          <div className="bg-card border rounded-xl p-5 shadow-sm space-y-4 mt-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Sucursal (Origen de Venta) *</label>
              <select 
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background"
                value={locationId}
            ...
            <div className="space-y-2 pt-2">
              ...
              <p className="text-xs text-muted-foreground">Requerido para generar la póliza al recibir el pago.</p>
            </div>
          </div>
        </div>
